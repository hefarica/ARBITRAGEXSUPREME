/**
 * ArbitrageX Supreme - Servidor de Notificaciones Multi-Canal
 * Ingenio Pichichi S.A. - Actividad 9.1
 * 
 * Servidor HTTP que expone API REST para el sistema de notificaciones
 * Integra con el servicio multi-canal y proporciona endpoints para:
 * - Envío de notificaciones
 * - Administración de canales y templates
...
// Importar el servicio de notificaciones (simulado por ahora ya que es TypeScript)
// En producción se compilaría el TypeScript o se usaría ts-node
...
class NotificationAPI {
  ...
  setupNotificationService() {
    // Simulación del servicio TypeScript para demostración funcional
...
📝 Mensaje: {{message}}`,
  ...
  setupRoutes() {
    ...
    fastify.get('/api/stats', async (request, reply) => {
      return {
        ...
        channels: Array.from(this.channels.values()).map(channel => ({
          ...
          sent: this.stats.channels[channel.id]?.sent || 0,
          failed: this.stats.channels[channel.id]?.failed || 0
        })),
        templates: Array.from(this.templates.values()).map(template => ({
          id: template.id,
          name: template.name,
          priority: template.priority,
          used: this.stats.templates[template.id]?.used || 0
        }))
      };
    });
    ...
    fastify.post('/api/notifications/send', {
      schema: {
        body: {
          type: 'object',
          required: ['templateId', 'recipient', 'variables'],
          properties: {
    ...
    // Envío rápido de alertas de arbitraje
      ...
      try {
        ...
        const message = {
          ...
          variables: {
            ...
            detailsUrl: 'https://arbitragex-supreme.com/opportunity/123'
          },
          channels: ['sendgrid-email', 'slack-webhook', 'discord-webhook'],
          priority: 'high'
        };
    ...
    fastify.post('/api/notifications/trading-alert', async (request, reply) => {
      try {
        ...
        const message = {
          ...
          recipient: recipient || { email: 'trader@arbitragex-supreme.com', phone: '+1234567890' },
          variables: {
            ...
            change: change || '+5.8',
            timestamp: new Date().toISOString(),
            description: `${alertType} activada para ${symbol}`,
            dashboardUrl: 'https://arbitragex-supreme.com/dashboard'
          },
          channels: ['sendgrid-email', 'twilio-sms', 'slack-webhook'],
          priority: 'critical'
        };
    ...
    // Test de conectividad de canales
    fastify.post('/api/channels/:channelId/test', async (request, reply) => {
      const { channelId } = request.params;
      const channel = this.channels.get(channelId);
      ...
      const testMessage = {
        ...
        recipient: { email: 'test@arbitragex-supreme.com', phone: '+1234567890' },
        variables: {
          severity: 'INFO',
          service: 'Notification Service',
          message: `Test de conectividad para canal ${channel.name}`,
          timestamp: new Date().toISOString(),
          recommendedActions: 'Ninguna - Test exitoso'
        },
        channels: [channelId],
        priority: 'low'
      };
      ...
      const results = await this.simulateSendNotification(testMessage, template);

      return {
        success: true,
        message: `Test de canal ${channel.name} completado`,
        results: results
      };
    });
    ...
    fastify.get('/dashboard', async (request, reply) => {
      const enhancedDashboard = this.generateEnhancedDashboardHTML();
      reply.type('text/html').send(enhancedDashboard);
    });
  }
  ...
  async simulateSendNotification(message, template) {
    ...
    for (const channelId of message.channels) {
      ...
      // Simular envío según el tipo de canal
      const result = {
        messageId: message.id,
                ...
                <div class="bg-gray-700 rounded-lg p-4">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-sm opacity-75">${template.subject || template.title}</p>
                        </div>
                ...
                const response = await fetch('/api/notifications/send', {
                    ...
                    body: JSON.stringify({
                        ...
                        variables: {
                            ...
                            timestamp: new Date().toISOString(),
                            recommendedActions: 'Monitor system resources'
                        }
                    })
                });
                const result = await response.json();
